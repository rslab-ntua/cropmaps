...
import logging
import datetime
from cropmaps.exceptions import NameNotInTimeSeriesError, MinMaxCloudBoundError, MinMaxDateError
        ...
        if cloud_coverage is True and date is True:
            ...
            for attr in attrs:
                if isinstance(getattr(self, attr), list):
                    ...
                    zipped = zip(self.cloud_cover, self.names, _l)
                    sorted_list = sorted(zipped, key = lambda k: (k[0], k[1]))
